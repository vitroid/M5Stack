...
int get_server_health()
{
  int status = -1;

  if ((wifi.run() == WL_CONNECTED)) {

    Serial.print("[HTTP] health...\n");
    http.begin("192.168.200.29", 8087, "/health"); // HTTP
    int httpCode = http.GET();
    // httpCode will be negative on error
    if (httpCode > 0) {
      Serial.printf("[HTTP] HEALTH... code: %d\n", httpCode);
      if (httpCode == HTTP_CODE_OK) {
        // tinyweb + prox.py are alive
        status = 0;
      } else if (httpCode == 503) {
        // tinyweb alive, prox.py heartbeat missing
        status = 1;
      } else {
        // some other HTTP-level error
        status = 2;
      }
    } else {
      Serial.printf("[HTTP] HEALTH... failed, error: %s\n", http.errorToString(httpCode).c_str());
    }
    http.end();
  }
  return status;
