...
int w = M5.Lcd.width();
int h = M5.Lcd.height(); //h = lcd.screensize()
int cx = w/2;
int cy = h/2;
...
void loop() {
  ...
  for(int i=-6;i <= 6; i++){
    ...
    if ( i % 2 != 0 )
      scale = 1;
    img.drawLine(cx-by*scale/ZOOM+px,cy-bx*scale/ZOOM-py,cx+by*scale/ZOOM+px,cy+bx*scale/ZOOM-py,0xffff);
    if ( i == 0 )
      continue;
    if ( i % 2 != 0 )
      continue;
    number(abs(i*5), cx+by*(scale+1)/ZOOM+px, cy+bx*(scale+1)/ZOOM-py, by/ZOOM, bx/ZOOM, 0xffff);
    number(abs(i*5), cx-by*(scale+4)/ZOOM+px, cy-bx*(scale+4)/ZOOM-py, by/ZOOM, bx/ZOOM, 0xffff);
  }
  // orange fixed
